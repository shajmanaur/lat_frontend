...
export default function MyAllocation() {
  ...
  const fetchAllocations = async () => {
    try {
      // Fetch allocations using the coordinator API
      const token = localStorage.getItem('token');
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001/api/v1';
      let allocRes;
      try {
        allocRes = await fetch(`${apiUrl}/teachers/allocations`, {
  ...
  const fetchStudentsForGrade = async (gradeKey: string) => {
    ...
    try {
      ...
      // Try fetching from the students list API
      try {
        const res = await studentsApi.getStudents({ page: 1, limit: 500 });
        let allStudents: any[] = [];
